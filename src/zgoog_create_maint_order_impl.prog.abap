...
*      ENDLOOP.
      ...
      /goog/cl_json_util=>deserialize_json(
        EXPORTING
          iv_json        = <ls_messages>
          iv_pretty_name = /ui2/cl_json=>pretty_mode-extended
        IMPORTING
      ...
      ls_order_data-order_type       = ls_message-order_type.
      ls_order_data-short_text       = ls_message-short_text.
      ls_order_data-equipment_number = ls_message-equipment_number.
      ls_order_data-work_center      = ls_message-work_center.
    ...
    SELECT objid,
      ...
      FOR ALL ENTRIES IN @lt_order_data
      WHERE objty = 'A' AND
            ktext = @lt_order_data-work_center.
    IF sy-subrc EQ 0.
      SELECT objid,
             arbpl
        FROM crhd
    ...
    LOOP AT lt_order_data ASSIGNING FIELD-SYMBOL(<ls_order_data>).
      ...
      TRY.
          DATA(ls_crtx) = VALUE #( lt_crtx[ ktext = <ls_order_data>-work_center ] ).
          DATA(ls_crhd) = VALUE #( lt_crhd[ objid = ls_crtx-objid ] ).
        CATCH cx_sy_itab_line_not_found.
      ENDTRY.
      ls_header-mn_wk_ctr  = ls_crhd-arbpl.
